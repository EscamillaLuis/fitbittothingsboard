...
def _is_valid_hr(bpm: Optional[float]) -> bool:
    ...
    try:
        ...
        return False
    return 30.0 <= b <= 220.0  # evita outliers
...
# --------- Parsing desde dict "data" del repo ---------
def parse_times_and_hr_from_data(data: Dict) -> Tuple[List[datetime], List[Optional[float]]]:
    """
    Extrae timestamps (datetime naive, día base) y HR (bpm) de data['Ritmo_Cardiaco']['dataset'].
    Asume data['Fecha']='YYYY-MM-DD' y cada item: {'time':'HH:MM:SS','value': bpm}
    """
...
def windowed_proxies(
    ...
    """
    Devuelve lista de dicts con: window_start, window_end (ISO),
    rmssd_proxy_ms, sdnn_proxy_ms, pnn50_proxy, n.
    """
